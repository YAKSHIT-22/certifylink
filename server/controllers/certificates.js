...
const sendCertificates = async (req, res) => {
  ...
  try {
    ...
    {
      $project: {
        organizations: { $ifNull: [{ $arrayElemAt: ["$organizations", 0] }, 0] },
    ...
    try {
      await data.map(async (item) => {
        ...
        dom.querySelector("#date").textContent = majorDetails.events.startDate === majorDetails.events.endDate ? majorDetails.events.endDate : `${majorDetails.events.startDate} to ${majorDetails.events.endDate}`;
        ...
        const sent = await sendMail(item, majorDetails.organizations, url, req.email);
        if (!sent) {
